...
namespace Tetris.Domain
{
    public class Field
    {
        ...
        public List<(Position,BlockTypes)> GetFieldBlockPointAndTypePairs()
        {
            var pairs = new List<(Position, BlockTypes)>();
            ...
            for (var row = 0; row < _height; row++)
            {
                for (var col = 0; col < _width; col++)
                {
                    ...
                    {
                        pairs.Add((new Position(col, row), _fieldTypeState[row, col]));
                    }
                }
            }
        ...
        public List<Position> GetFieldBlockPoints()
        {
            ...
            for (var row = 0; row < _height; row++)
            {
                for (var col = 0; col < _width; col++)
                {
                    if (_fieldState[row, col] != FieldTypes.empty)
                    {
                        points.Add(new Position(col, row));
                    }
                }
            }
        ...
        public List<Position> GetFixedBlockPoints()
        {
            var points = new List<Position>();
        ...
        public int CountCollisionPoint(List<Position> points)
        {
