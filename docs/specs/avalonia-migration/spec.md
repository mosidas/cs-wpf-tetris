...
- Domain / Application / Infrastructure の**ゲームロジック挙動**の変更 — 理由: ロジックは前段で凍結済みで、本作業は挙動不変のプレゼンテーション差し替えに限定する(既存の公開 API を利用するのみ)。ただし例外として、WPF 撤去により消費者が消滅する互換ファサード `Tetris.Application.GameManager`・`ActionTypes` と、それらのみを対象とする `GameManagerTests` の撤去は、挙動不変のデッドコード整理として本作業単位に含める(ゲームロジック=`GameSession`・コマンド・スコア・7-bag は不変。§8・§7 R9.3 参照)。
...
- 既存契約(参照のみ): `Tetris.Application/GameSession.cs`、`GameStateSnapshot.cs`、`GameCommand.cs`、`IBlocksPoolManager.cs`、`Tetris.Infrastructure/BlocksPoolManager.cs`、`Tetris.Domain/*`。
- 撤去対象(WPF 撤去で消費者ゼロとなる互換ファサード。挙動不変のデッドコード整理): `Tetris.Application/GameManager.cs`、`Tetris.Application/ActionTypes.cs`、`Tetris.Application.Tests/GameManagerTests.cs`。ゲーム進行は §8 のとおり `GameSession` を直接利用する。
